...
namespace TcpListenerApp {
  public class TcpClientService {
    ...
    public async Task Run() {
      ...
      using (var stream = _client.GetStream()) {
        ...
        while ((length = await stream.ReadAsync(bytes, 0, bytes.Length)) != 0) {
          Log.Info(string.Format("{0} - received [{1}]", DateTime.Now, DecodeByteArrayHexToAscii(bytes.Take(length).Select(x => x.ToString("X2")).ToArray())));
    ...
    public static string DecodeByteArrayHexToAscii(string[] byteStrings) {
      try {

        var hexString = String.Join("", byteStrings);

        string ascii = string.Empty;
        for (int i = 0; i < hexString.Length; i += 2) {
          String hc = string.Empty;

          hc = hexString.Substring(i, 2);
          uint decval = System.Convert.ToUInt32(hc, 16);
